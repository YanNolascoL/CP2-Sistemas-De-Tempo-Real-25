...
#define STUDENT_INFO "{Yan-Landin-RM85774}"
...
void init_task_watchdog(void)
{
    ...
    printf(STUDENT_INFO " [WDT] Watchdog inicializado com timeout de %d segundos.\n", WDT_TIMEOUT_S);
}
...
void task_generate(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (sent == pdPASS)
        {
            printf(STUDENT_INFO " [FILA] Dado %d enviado com sucesso!\n", value);
        }
        else
        {
            printf(STUDENT_INFO " [FILA] Fila cheia, dado %d descartado!\n", value);
        }
...
void task_receive(void *pvParameters)
{
    ...
    while (1)
    {
        if (xQueueReceive(dataQueue, &receivedValue, pdMS_TO_TICKS(RECEPTION_TIMEOUT_MS)) == pdPASS)
        {
            ...
            {
                *ptr = receivedValue;
                printf(STUDENT_INFO " [RECEPÇÃO] Valor recebido: %d\n", *ptr);
                free(ptr);
            }
            else
            {
                printf(STUDENT_INFO " [RECEPÇÃO] Erro ao alocar memória!\n");
            }
        ...
        {
            printf(STUDENT_INFO " [RECEPÇÃO] Nenhum dado recebido no tempo limite!\n");
            printf(STUDENT_INFO " [RECEPÇÃO] Tentando recuperar...\n");

            vTaskDelay(pdMS_TO_TICKS(1000));

            if (xQueueReceive(dataQueue, &receivedValue, pdMS_TO_TICKS(1000)) == pdPASS)
            {
                printf(STUDENT_INFO " [RECEPÇÃO] Recuperação bem-sucedida! Valor: %d\n", receivedValue);
                reception_ok = true;
            }
            else
            {
                printf(STUDENT_INFO " [RECEPÇÃO] Falha persistente. Encerrando tarefa...\n");
                reception_ok = false;
                vTaskDelete(NULL);
            }
        }
...
void task_supervision(void *pvParameters)
{
    ...
    while (1)
    {
        printf("\n" STUDENT_INFO " [SUPERVISÃO] STATUS DO SISTEMA:\n");
...
void app_main(void)
{
    printf(STUDENT_INFO " [SISTEMA] Inicializando sistema multitarefa...\n");
    ...
    if (dataQueue == NULL)
    {
        printf(STUDENT_INFO " [ERRO] Falha ao criar fila!\n");
        return;
    }
    ...
    printf(STUDENT_INFO " [SISTEMA] Tarefas iniciadas com sucesso!\n");
}